...
async function fetchWorkspace() {
  const response = await apiFetch(`${BASE_URL}/workspace`, { headers: authHeader() });
  if (response) {
...
async function fetchUsers() {
  loading.value = true;
  const response = await apiFetch(`${BASE_URL}/users`, { headers: authHeader() });
  if (response) {
    const data = await response.json();
...
<template>
  <div class="min-h-screen bg-[var(--color-surface)]">
    ...
    <nav class="h-14 bg-[var(--color-card)] border-b border-[var(--color-border)] px-4 flex items-center justify-between sticky top-0 z-30">
      <button @click="router.back()" class="w-9 h-9 flex items-center justify-center text-[var(--color-primary-light)] active:scale-90 transition-transform">
    ...
    <main class="max-w-md mx-auto px-4 pt-6 pb-12 flex flex-col gap-6">
      ...
      <div class="bg-[var(--color-card)] rounded-2xl border border-[var(--color-border)] p-5 flex flex-col gap-4">
        <h2 class="text-sm font-black tracking-widest uppercase text-[var(--color-text-muted)]">Workspace</h2>

        <!-- Workspace var -->
        <div v-if="workspace" class="flex flex-col gap-3">
          <div class="flex items-center justify-between">
            <span class="font-bold text-[var(--color-text-primary)]">{{ workspace.name }}</span>
            <span class="text-xs text-green-500">Aktif</span>
          </div>

          <!-- Davet kodu -->
          <div class="flex items-center gap-2 bg-[var(--color-surface)] rounded-xl px-4 py-3">
            <span class="text-xs text-[var(--color-text-muted)]">Davet Kodu:</span>
            <span class="flex-1 font-black text-[var(--color-primary-light)] tracking-widest">{{ workspace.invite_code }}</span>
            <button
        ...
        <div v-else class="flex flex-col gap-3">
          <div class="flex flex-col gap-2">
            <p class="text-xs text-[var(--color-text-secondary)]">Yeni workspace oluştur:</p>
            <div class="flex gap-2">
              <input v-model="newWorkspaceName" type="text" placeholder="Workspace adı"
              ...
              <button @click="createWorkspace" :disabled="!newWorkspaceName || workspaceLoading"
          ...
          <div class="flex flex-col gap-2">
            <p class="text-xs text-[var(--color-text-secondary)]">Davet kodu ile katıl:</p>
            <div class="flex gap-2">
              <input v-model="inviteCode" type="text" placeholder="Davet kodu"
              ...
              <button @click="joinWorkspace" :disabled="!inviteCode || workspaceLoading"
                class="px-4 py-2 rounded-xl bg-indigo-700 text-white text-sm font-bold disabled:opacity-40">
                Katıl
              </button>
            </div>
      ...
      <!-- Yeni Worker -->
      <div v-if="workspace" class="bg-[var(--color-card)] rounded-2xl border border-[var(--color-border)] p-5 flex flex-col gap-4">
        <h2 class="text-sm font-black tracking-widest uppercase text-[var(--color-text-muted)]">Yeni Worker</h2>
      ...
      <!-- Çalışanlar -->
      <div v-if="workspace" class="bg-[var(--color-card)] rounded-2xl border border-[var(--color-border)] p-5 flex flex-col gap-3">
        <h2 class="text-sm font-black tracking-widest uppercase text-[var(--color-text-muted)]">Üyeler</h2>
        <div v-if="loading" class="text-center py-4 text-[var(--color-text-muted)] text-sm">Yükleniyor...</div>
        <div v-else-if="users.length === 0" class="text-center py-4 text-[var(--color-text-muted)] text-sm">Henüz çalışan yok</div>
